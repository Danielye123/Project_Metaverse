...
const nebulaFieldNode = Fn(([rayDir]) => {
...
const blackHoleShader = Fn(() => {
  ...
  If(escaped.greaterThan(0.5).and(alpha.lessThan(0.99)), () => {
    ...
    If(uniforms.starsEnabled.greaterThan(0.5), () => {
      bgColor.addAssign(starFieldNode(rayDir));
    });
    If(uniforms.nebulaEnabled.greaterThan(0.5), () => {
      bgColor.addAssign(nebulaFieldNode(rayDir));
    });
...
const FAR_DISTANCE = 85;
const ELEVATION = -0.111;
const ROTATE_SPEED = 0.03;

const BlackHole = ({ progressRef }) => {
  ...
  useEffect(() => {
    ...
    // Scroll-driven dolly: the camera pulls back as the section scrolls past,
    // so the black hole recedes into the starfield and shrinks away.
    const positionCamera = () => {
      const raw = progressRef && typeof progressRef.current === 'number' ? progressRef.current : 0;
      const progress = Math.min(Math.max(raw, 0), 1);
      const distance = CLOSE_DISTANCE + (FAR_DISTANCE - CLOSE_DISTANCE) * progress;
      const azimuth = Math.PI + uniforms.time.value * ROTATE_SPEED;
    ...
    renderer.init().then(() => {
      ...
      // Always draw one frame so the canvas is never blank while waiting on
      // the viewport-gated loop to start.
      positionCamera();
      renderFrame();
  ...
  if (unsupported) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div
          className="w-[55vmin] h-[55vmin] rounded-full"
          style={{
            background: 'radial-gradient(circle, #000 36%, var(--color-accent) 39%, rgba(0,0,0,0) 62%)',
            boxShadow: '0 0 140px 10px var(--color-glow)',
          }}
        />
      </div>
    );
